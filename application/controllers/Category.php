	...
	public function create()
	{
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	if($create == true) {
        		...
        		$response['messages'] = 'Creada Exitosamente';
        	}
        	else {
        		$response['success'] = false;
        		$response['messages'] = 'Error en la Base de Datos al introducir datos';
        	}
        }
        else {
        	$response['success'] = false;
		...
		if($id) {
	        ...
	        if ($this->form_validation->run() == TRUE) {
	        	...
	        	if($update == true) {
	        		...
	        		$response['success'] = false;
	        		$response['messages'] = 'Error en la base de datos al actualizarlos';
	        	}
	        }
	        else {
	        	$response['success'] = false;
			...
			$response['success'] = false;
    		$response['messages'] = 'Error por favor refresque la página';
		}
	...
	public function remove()
	{
		...
		if($category_id) {
			...
			if($delete == true) {
				...
				$response['success'] = false;
				$response['messages'] = "Error en la base de datos mientras se eliminaba";
			}
		}
		else {
			$response['success'] = false;
			$response['messages'] = "Refresque la página de nuevo.";
		}
